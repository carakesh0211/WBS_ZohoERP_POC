...
| 14 | 2026-09-14 02:1x IST (2026-09-13 20:5x UTC) | `63b07c6`, `ae0ccf86011dd4961e01cc6438a5c658c2be17e16ba35b756164bdac80623b1e` | standalone (as 2) | the full-application build of streams A-G on one tree: internal material fulfilment (034), the Administrator's audited self-approval override, .xlsx on every export dataset (035), two sign-in choices with forgot/reset, policy, history and break-glass (036), the durable notification outbox (036), collapsible navigation, and their screens; the adversarial review's 15 findings closed. Gates before it: CI green on all five jobs at this head (regression 5012 passed / 826 intended skips; live PostgreSQL 1864 passed / 2 declared skips; contract, supply chain, VRT), the bounded VRT release plan run twice locally (run 1 enumerated, one explained re-baseline pass, run 2 confirmed), bundle scanned clean (24 manylinux wheels incl. openpyxl). `migrate_pg --status` 033 before, `--upgrade` applied 034 035 036, 036 after, each in its own transaction, the running a65841f instance stayed `/readyz` 200 at 036 throughout. After it: `/` 200, `/healthz` 200, `/readyz` 200 `036`, `/api/health` ok with writes enabled (the standing LIVE_WRITE state, unchanged), `/docs` 404, `/api/auth/providers` reports the WBS choice with forgot-password on and the Zoho choice OFF until the owner configures it, banner unchanged; role smoke 13/13 identities incl. the four ERP-linked Administrators, every check passed (`evidence/release/smoke-roles-release-2026-09-14.json`; the smoke now counts the deliberate `CONTROL_TOTALS_UNAVAILABLE` 503 as the documented answer it is); the post-deploy cycle driver re-ran the read sweeps and the idempotent adoption (`evidence/e2e/after-deploy-2026-09-13T210446.json`: adopted 0, linked 0, skipped 7, the same two exceptions; no ERP write); one `.xlsx` export created, driven and downloaded as U-ADM and inspected (`evidence/release/exports-2026-09-14/`: four sheets, typed cells, UTC datetimes, Indian money mask, SUM totals). No credential appeared in any response. One deployment, no corrective second. |
