...
#: ...and 016 corrects 014 in turn, for the same additive reason. 014's
...
#: line could hold exactly one of them for ever. 016 keeps every column and
...
ORDINAL = (_Path(__file__).resolve().parents[1] / "migrations" / "pg"
           / "016_grn_line_ordinal.sql").read_text(encoding="utf-8")
...
def test_the_grn_line_conflict_target_matches_the_migrations_constraint():
    ...
    # ...and 016 is the index that is actually THERE, so it is what the
...
def test_replaying_a_line_without_a_line_id_duplicates_nothing(seeded):
    ...
    016 KEEPS IT CLOSED. The row HAS a `receive_external_id` --
    `record_receive_line` refuses a blank one before writing anything -- so it
    falls inside 016's `WHERE receive_external_id IS NOT NULL` predicate and
    014's rule applies to it in full. This is the half of the tension that
    016's partiality must not give back.
    """
    session = _session(seeded)
    for _pass in range(3):
...
def test_every_money_moving_success_path_writes_an_audit_entry():
    ...
    """
    # THE PATHS THAT MUST AUDIT DIRECTLY.
    for name in ("_settle_receive_line", "mirror_bill", "retract_quarantine"):
        assert "_audit(" in _sql(name), (
            name + " moves money and writes no audit entry")

    # `_supersede_withdrawn_bill_lines` IS AUDITED BY ITS SOLE CALLER, and
    # that is stated here rather than dropped from the list.
    #
    # It originally carried its own `_audit(` loop. Migration 014 split the
    # audit into `_audit_superseded_bill_lines` because the UPDATE now has to
    # RETURN the cell keys so the ledger can be refreshed -- so this test went
    # red while the audit entry was still, in fact, written.
    #
    # Removing the function from the list above would have been the easy fix
    # and the wrong one: it would stop asserting anything about a step that
    # zeroes money. Instead BOTH halves of the arrangement are pinned, so the
    # entries cannot stop being written without something here failing:
    #   * the helper must actually audit, and
    #   * `mirror_bill` -- its only caller -- must actually call it.
    assert "_audit(" in _sql("_audit_superseded_bill_lines"), (
        "the superseded-line audit helper no longer writes an entry")
    assert "_audit_superseded_bill_lines(" in _sql("mirror_bill"), (
        "mirror_bill no longer calls the superseded-line audit helper, so "
        "those entries are never written")
    assert "_supersede_withdrawn_bill_lines(" in _sql("mirror_bill"), (
        "mirror_bill no longer supersedes withdrawn lines; if that moved "
        "elsewhere the audit arrangement above moved with it and this test "
        "is now pinning the wrong caller")
