"""Migration 017 and the export machinery: chunking, money, determinism.
...
DATABASE -- those are properties of migration 017's TEXT, of the runner's own
...
_017_PATH = MIGRATIONS / "018_export_jobs.sql"
_017 = _017_PATH.read_text(encoding="utf-8")

#: 017 with its prose removed. Every "the migration does not do X" assertion
...
_017_APPLIED = _017_CODE[:_017_CODE.rindex("COMMIT;")]
...
# =========================================================================
# Migration 017: it is a migration, it is additive, and it is complete
# =========================================================================
def test_017_is_discovered_named_and_numbered():
    versions = [m.version for m in migrate_pg.discover()]
    assert versions == sorted(versions)
    assert versions.count("017") == 1
    assert _migration("017").name == "export_jobs"
...
def test_017_does_not_touch_an_already_applied_migration(earlier):
    ...
    assert "export_job" not in text, (
        f"{earlier} mentions export_job; 017 must be additive and must not "
        f"have been produced by editing an applied migration")


def test_017_creates_only_its_own_two_tables():
    created = re.findall(r"^CREATE TABLE (\w+)", _017_APPLIED, flags=re.MULTILINE)
    assert created == ["export_job", "export_job_chunk"]


def test_017_names_every_constraint_it_creates():
    ...
    assert unnamed == [], f"017 carries unnamed constraints: {unnamed}"
...
def test_017_declares_the_named_constraint(name):
    assert f"CONSTRAINT {name}" in _017_APPLIED


def test_017_makes_the_captured_authorisation_immutable_in_the_database():
    ...
    assert "CREATE TRIGGER trg_export_job_identity_immutable" in _017_APPLIED
    guard = _017_APPLIED[_017_APPLIED.index("export_job_identity_is_immutable()"):]
...
def test_017_makes_chunks_append_only_by_privilege_as_well_as_by_trigger():
    ...
    assert "CREATE TRIGGER trg_export_job_chunk_append_only" in _017_APPLIED
    assert "REVOKE UPDATE ON export_job_chunk FROM capex_app;" in _017_APPLIED
    assert "REVOKE DELETE ON export_job FROM capex_app;" in _017_APPLIED
    # ...and DELETE on a CHUNK is granted on purpose: expiry purges bytes while
    # the job row recording who exported what stays.
    assert "GRANT DELETE ON export_job_chunk TO capex_app;" in _017_APPLIED


def test_017_enables_forces_and_policies_both_tables():
    for table in ("export_job", "export_job_chunk"):
        assert f"ALTER TABLE {table} ENABLE ROW LEVEL SECURITY" in _017_APPLIED
        assert f"ALTER TABLE {table} FORCE ROW LEVEL SECURITY" in _017_APPLIED
        assert re.search(rf"CREATE POLICY \w+ ON {table}\b", _017_APPLIED)


def test_017_declares_a_rollback_naming_every_policy_and_trigger_it_creates():
    assert "-- ROLLBACK:" in _017
    rollback = _017.split("-- ROLLBACK:", 1)[1]
    for policy, table in re.findall(r"^CREATE POLICY (\w+) ON (\w+)",
                                    _017_APPLIED, flags=re.MULTILINE):
        assert f"DROP POLICY IF EXISTS {policy} ON {table};" in rollback
    for trigger, table in re.findall(
            r"CREATE TRIGGER (\w+) BEFORE UPDATE ON (\w+)", _017_APPLIED):
        assert f"DROP TRIGGER IF EXISTS {trigger} ON {table};" in rollback


def test_017_does_not_redefine_the_frozen_scope_functions():
    ...
    for function in ("capex_scope_permits", "capex_dimension_permits",
        ...
        assert not re.search(
            rf"CREATE\s+(OR\s+REPLACE\s+)?FUNCTION\s+{function}\b", _017_CODE)
    assert "capex_principal_present()" in _017_APPLIED


def test_017_carries_no_paise_column():
    ...
    assert "_paise" not in _017_APPLIED
...
def test_expiry_purges_the_bytes_and_keeps_the_record(pg_database):
    ...
    matters after the rendered rows are gone -- and 017 revokes DELETE on
...
def test_the_result_is_not_served_if_its_bytes_no_longer_match_the_digest(
        pg_database):
    """017 makes chunks append-only, so a mismatch means something reached past
