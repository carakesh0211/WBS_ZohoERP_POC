...
MUTATING_ROUTES = [
    ...
    ("/api/budget/revisions/{revision_id}/approve", "POST",
     ...
     "revision.approve", "Requestor"),
    # Fable 5.1: original-budget creation and the budget CATEGORY master
    # (migration 026). Drafting/submitting/cancelling/importing carry the
    # maker's `budget.create`; the category master carries
    # `budget.category.manage`. Approval has NO direct route: it is the
    # approval engine's `/api/approvals/{instance_id}/decide`.
    ("/api/budget/originals", "POST", "/api/budget/originals",
     {"project_id": "PRJ-DM-001", "fiscal_year": "2026-27", "title": "unauthorised attempt",
      "lines": [{"wbs_id": "WBS-A-CIVIL", "budget_head_id": "BH-DM1-CIVIL",
                 "budget_category_id": "BC-CIVIL", "amount_paise": 100000}]},
     "budget.create", "ProcurementApprover"),
    ("/api/budget/originals/{budget_id}", "PUT", "/api/budget/originals/OB-NONE",
     {"expected_version": 1, "title": "unauthorised attempt"},
     "budget.create", "ProcurementApprover"),
    ("/api/budget/originals/{budget_id}/submit", "POST", "/api/budget/originals/OB-NONE/submit",
     {}, "budget.create", "ProcurementApprover"),
    ("/api/budget/originals/{budget_id}/cancel", "POST", "/api/budget/originals/OB-NONE/cancel",
     {"reason": "unauthorised attempt"}, "budget.create", "ProcurementApprover"),
    ("/api/budget/originals/import/preview", "POST", "/api/budget/originals/import/preview",
     {"project_id": "PRJ-DM-001", "csv_text": "wbs_code,budget_head_code,category_code,amount_rupees\n"},
     "budget.create", "ProcurementApprover"),
    ("/api/budget/originals/import", "POST", "/api/budget/originals/import",
     {"project_id": "PRJ-DM-001", "fiscal_year": "2026-27", "title": "unauthorised attempt",
      "csv_text": "wbs_code,budget_head_code,category_code,amount_rupees\n"},
     "budget.create", "ProcurementApprover"),
    ("/api/budget/categories", "POST", "/api/budget/categories",
     {"code": "UNAUTH", "name": "unauthorised attempt"},
     "budget.category.manage", "ProcurementApprover"),
    ("/api/budget/categories/{category_id}", "PUT", "/api/budget/categories/BC-NONE",
     {"expected_version": 1, "name": "unauthorised attempt"},
     "budget.category.manage", "ProcurementApprover"),
    ...
    ("/api/procurement/purchase-orders", "POST",
     ...
     {"project_id": "PRJ-01", "vendor_name": "unauthorised attempt",
      "lines": [{"wbs_id": "W-03-01", "budget_head_id": "BH-PM",
                 "amount_paise": 100000}]},
     "po.amend", "Requestor"),
    # Fable 5.1 / 034: internal material fulfilment. The router floor is
    # `budget.read` again; every denied role below clears it and stops at the
    # route's own permission. Auditor holds `imr.read` only.
    ("/api/procurement/purchase-requests/{pr_id}/lines/{pr_line_id}/fulfilment",
     "PUT", "/api/procurement/purchase-requests/PR-015/lines/PRL-1/fulfilment",
     {"mode": "INTERNAL_TRANSFER", "internal_quantity": 1},
     "fulfilment.decide", "Requestor"),
    ("/api/procurement/internal-material-requests", "POST",
     "/api/procurement/internal-material-requests",
     {"pr_line_id": "PRL-1", "quantity": 1},
     "imr.create", "Auditor"),
    ("/api/procurement/internal-material-requests/{imr_id}/approve", "POST",
     "/api/procurement/internal-material-requests/IMR-1/approve", {},
     "imr.approve", "Requestor"),
    ("/api/procurement/internal-material-requests/{imr_id}/valuation", "PUT",
     "/api/procurement/internal-material-requests/IMR-1/valuation",
     {"unit_rate_paise": 100, "note": "unauthorised attempt"},
     "imr.allocate", "Requestor"),
    ("/api/procurement/internal-material-requests/{imr_id}/allocate", "POST",
     "/api/procurement/internal-material-requests/IMR-1/allocate",
     {"idempotency_key": "k"}, "imr.allocate", "Requestor"),
    ("/api/procurement/internal-material-requests/{imr_id}/issue", "POST",
     "/api/procurement/internal-material-requests/IMR-1/issue",
     {"quantity": 1, "idempotency_key": "k"}, "imr.issue", "Requestor"),
    ("/api/procurement/internal-material-requests/{imr_id}/return", "POST",
     "/api/procurement/internal-material-requests/IMR-1/return",
     {"quantity": 1, "idempotency_key": "k"}, "imr.issue", "Requestor"),
    ("/api/procurement/internal-material-requests/{imr_id}/consume", "POST",
     "/api/procurement/internal-material-requests/IMR-1/consume",
     {"quantity": 1, "idempotency_key": "k"}, "imr.issue", "Requestor"),
    ("/api/procurement/internal-material-requests/{imr_id}/transfer", "POST",
     "/api/procurement/internal-material-requests/IMR-1/transfer",
     {"quantity": 1, "to_location_id": "LOC-2", "idempotency_key": "k"},
     "imr.issue", "Requestor"),
    ("/api/procurement/internal-material-requests/{imr_id}/cancel", "POST",
     "/api/procurement/internal-material-requests/IMR-1/cancel",
     {"reason": "unauthorised attempt"}, "imr.cancel", "Requestor"),
    ...
    # for `None` rows, so
    # `tests/test_api_auth_saved_views.py` asserts it directly for these
    # five. It lives in its own file, not below, because test_api_auth.py
    # is one of the files the 220-function baseline is measured over and
    # adding to it moves the fixed reference the manifest exists to hold.
    #
    # THAT SENTENCE USED TO NAME `tests/test_pg_reporting.py`, AND IT WAS
    # FALSE. That file asserts nothing of the kind, and being PostgreSQL-only
    # it skips on every machine without a server -- so the claim could sit here
    # unexamined indefinitely. It also rested on 017's `WITH CHECK` pinning
    # `owner_user_id`, which PostgreSQL never consults for DELETE and which
    # nothing makes immutable on UPDATE.
    #
    # The conclusion still holds for a PRIVATE view, which is why the floor is
    # still the right gate here: it is a bookmark under the caller's own
    # identity. What was genuinely ungated was SHARING -- `visibility='SHARED'`
    # publishes a view other people open and read money through -- and that now
    # requires `report.view.share` inside the handler, because the visibility
    # arrives in the body where a route-level dependency cannot see it.
